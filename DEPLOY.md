...
Gate API-ключи опциональны (только для точных taker/funding-комиссий). Alchemy-ключи
остались от прошлой DEX-версии и рабочим CEX-путём не используются. Проверить
`EXCLUDE_SYMBOLS=BTC,ETH` и параметры `OFM_*`/`SCAN_*`.
